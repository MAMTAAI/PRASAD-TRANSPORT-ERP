...
import {
  ...
  AlertTriangle, CheckCircle2, Loader2, Undo2, Wand2, ChevronRight, Building2,
} from 'lucide-react';
...
export default function FleetCardSettlement() {
  ...
  // Client-side text filter. The list is already bounded server-side, and a
  // clerk typing a lorry number wants it now, not after a round trip.
  const shown = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return queue;
    return queue.filter((r) =>
      [r.vehicle_no, r.vehicle_raw, r.merchant_name, r.account_no]
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { k: 'Clearing me', v: inr(waiting), s: `${total.rows} swipe intezaar me`, i: Split, c: 'text-amber-300' },
      ...
      <GlassPanel>
        <PanelHeader
          ...
          sub={`${total.rows} swipe · ${inr(total.amount)} clearing me`}
        ...
        {loading ? (
          <div className="flex items-center gap-2 px-4 pb-6 text-slate-400">
            <Loader2 size={16} className="animate-spin" /> Queue khul rahi hai…
          </div>
        ) : shown.length === 0 ? (
          <div className="px-4 pb-6 text-[13px] text-slate-400">
            {total.rows === 0
          ...
          <div className="overflow-x-auto px-4 pb-4">
            <table className="w-full min-w-[820px] text-[12.5px]">
              <thead>
                <tr className="border-b border-slate-700 text-left text-[10px] uppercase tracking-[0.1em] text-slate-500">
                  <th className="py-2 pr-3 font-semibold">Tareekh</th>
                  <th className="py-2 pr-3 font-semibold">Lorry</th>
                  <th className="py-2 pr-3 font-semibold">Pump</th>
                  <th className="py-2 pr-3 text-right font-semibold">Litre</th>
                  <th className="py-2 pr-3 text-right font-semibold">Bacha hua</th>
                  <th className="py-2 pr-3 font-semibold">Kyun ruka hai</th>
                ...
                {shown.map((r) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-3 whitespace-nowrap font-mono text-slate-200">
                      {r.vehicle_no || <span className="text-slate-500">{r.vehicle_raw || '—'}</span>}
                    </td>
            ...
            {shown.length < total.rows && (
              <div className="pt-3 text-[11.5px] text-slate-500">
                {shown.length} dikh rahe hain, kul {total.rows}. Filter lagayein.
              </div>
            )}
          </div>
        )}
