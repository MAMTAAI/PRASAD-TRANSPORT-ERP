...
export default function OwnerBills({ api, apiJson, periodFrom, onNeedRate, onChanged }) {
  ...
  const buildAll = async () => {
    ...
    if (!window.confirm('Draft / refresh the bills of every fortnight from 1 Apr 2026 to today?' + NL
      + 'Bills already reviewed or approved are not touched.')) return;
    setBusy(true);
    try {
      ...
      alert(`🤖 TARA covered ${j.periods?.length ?? 0} fortnight(s): ${tot.bills} bills, ${tot.c} new lorry drafts, ${tot.r} refreshed.`);
  ...
  return (
    <div className="glass-card" style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap', alignItems: 'flex-start', marginBottom: '12px' }}>
        <div>
          ...
          <div style={{ color: C.mut, fontSize: '12.5px', marginTop: '3px' }}>
            One bill per owner in the IOCL layout — click a row to open the full bill. TARA drafts on the 1st and 16th
            at 03:00; the desk corrects it, an admin approves and posts it to the owner's ledger.
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          <span onClick={() => setOnlyCycle(true)} style={chip(onlyCycle)}>This cycle only</span>
          <span onClick={() => setOnlyCycle(false)} style={chip(!onlyCycle)}>All cycles</span>
          ...
          <span style={{ width: '6px' }} />
          {[['', 'All'], ['AGENCY', 'Attached'], ['OWN', 'Own vehicles'], ['MARKET', '🛒 Market (fleet partner)']].map((s) => (
            <span key={s[0]} onClick={() => setCls(s[0])} style={chip(cls === s[0])}>{s[1]}</span>
          ))}
          <input value={owner} onChange={(e) => setOwner(e.target.value)} placeholder="Search owner / bill no"
          ...
          <button onClick={buildAll} disabled={busy} style={btn('ai', !busy)} title="1 Apr 2026 se aaj tak, admin">
            {busy ? '⏳ Building…' : '📅 Draft all since April'}
          </button>
        </div>
      </div>
      ...
      {data.cards.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px', margin: '0 0 14px' }}>
          {data.cards.slice(0, 6).map((c) => {
            ...
            return (
              <div key={c.owner_key + c.class_key} onClick={() => setOwner(c.owner_name)} title="Show this owner's bills"
                ...
                <div style={{ fontSize: '12px', color: C.ink2, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {c.owner_name}{!agency && <span style={{ color: C.dim, fontWeight: 500 }}> · own vehicle</span>}
                </div>
                <div style={{ fontSize: '21px', fontWeight: 800, marginTop: '2px', fontVariantNumeric: 'tabular-nums',
                  ...
                  {!agency ? 'P&L' : owed > 0 ? inr(owed) : n2(c.needs_rate) > 0 ? 'rate missing' : inr(c.pending_payable)}
                </div>
                <div style={{ fontSize: '10.5px', color: C.dim, marginTop: '3px' }}>
                  {agency
                    ? `${c.approved} approved · payable ${inr(owed)} · in draft ${inr(c.pending_payable)}${n2(c.needs_rate) > 0 ? ` · ${c.needs_rate} lorry without rate` : ''}`
      ...
      <div style={{ overflowX: 'auto', border: `1px solid ${C.line}`, borderRadius: '10px' }}>
        {loading ? (
          <p style={{ color: C.warn, textAlign: 'center', padding: '26px' }}>Loading bills…</p>
        ) : data.rows.length === 0 ? (
          <p style={{ color: C.dim, textAlign: 'center', padding: '26px', fontSize: '13px' }}>
            No bills yet — use "🤖 Build drafts" or "📅 Draft all since April" above.
          </p>
        ) : (
          <table style={{ width: '100%', minWidth: '1080px', borderCollapse: 'collapse', fontSize: '12.5px' }}>
            <thead>
              <tr>
                <th style={th}>Bill No</th><th style={th}>Owner / Partner</th><th style={th}>Cycle</th>
                <th style={{ ...th, textAlign: 'right' }}>Lorry / Truck</th><th style={{ ...th, textAlign: 'right' }}>Trip / Load</th>
                <th style={{ ...th, textAlign: 'right' }}>Freight</th><th style={{ ...th, textAlign: 'right' }}>Total expenses / Partner freight</th>
            ...
            <tbody>
              {pg.slice.map((b) => {
                ...
                return (
                  <tr key={b.id} onClick={() => setOpenId(b.id)} style={{ cursor: 'pointer', background: market ? 'rgba(244,114,182,0.04)' : 'transparent' }} title="Click to open the bill">
                    ...
                    <td style={{ ...td, color: C.ink, whiteSpace: 'normal', minWidth: '200px' }}>
                      ...
                      <div style={{ fontSize: '10.5px', color: C.dim }}>
                        {market ? `🛒 MARKET · fleet partner · books of ${b.company_name ?? ''}`
                          : agency ? `${b.class_key} · books of ${b.company_name ?? b.operating_company ?? ''}` : `OWN · nothing payable, P&L only`}
                      </div>
                    </td>
                    ...
                    <td style={{ ...tdR, fontWeight: 700, color: agency ? (b.payable === null ? C.dim : C.good) : (net >= 0 ? C.good : C.crit) }}>
                      {agency ? (b.payable === null ? '—' : inr2(b.payable)) : `${net >= 0 ? 'profit' : 'loss'} ${inr(Math.abs(net))}`}
                    </td>
                    <td style={td}>
      ...
      <p style={{ color: C.dim, fontSize: '11px', marginTop: '10px', lineHeight: 1.6 }}>
        Bill no = owner initials + fortnight (e.g. AFS-JUL-H2-2026). Total expenses = HSD + Toll + Fooding + Fixed +
        Doc + Other (+ Trip Advance on an attached lorry). RATE? = no rate for that lorry in the Commission Master —
        the bill cannot be approved until one is entered.
      </p>
...
function BillDrawer({ api, apiJson, id, onClose, onNeedRate, onChanged }) {
  ...
  const payBalance = async () => {
    ...
    const account = window.prompt(`Pay ${b.owner_name} the balance ${inr2(b.payable)} from which bank / cash account? (ledger name)` + hint,
      accounts[0]?.ledger_name ?? '');
    if (!account) return;
    if (!window.confirm(`${inr2(b.payable)} → ${b.owner_name}, "${account}" se.` + NL
      + `One PAYMENT voucher will be posted, ${b.loads} load(s) marked SETTLED and the partner notified on WhatsApp. Continue?`)) return;
    setBusy(true); setErr('');
    try {
      const j = await apiJson(`${api}/${id}/pay-balance`, { method: 'POST', body: JSON.stringify({ account }) });
      alert(`💸 Paid — ${inr2(j.amount)}. ${j.loads_settled} load(s) settled.`);
      await load(); onChanged?.();
    } catch (e) { setErr(e?.code === 'FORBIDDEN' ? 'Only an admin can pay the balance.' : (e?.message ?? 'Payment failed')); }
    setBusy(false);
  };
  ...
  const save = async () => {
    ...
    if (market) {
      try {
        ...
        alert('💾 Saved. Nothing posted to the ledger — TDS posts on approval.');
    ...
    try {
      ...
      await load(); onChanged?.();
      setEditing(false);
      alert(`💾 Saved. ${entriesOut.length ? entriesOut.length + ' expense(s) recorded under their trips. ' : ''}Nothing posted to the ledger — it posts on approval.`);
      void j;
    } catch (e) { setErr(e?.message ?? 'Save failed'); }
    setBusy(false);
  };
  ...
  const approve = async () => {
    ...
    if (n2(b.needs_rate) > 0) {
      if (market) { alert('⚠️ The partner\'s TDS rate is not known — choose "Individual or Firm" in the Fleet Partner master (or tick the 194C(6) declaration).'); return; }
      alert(`⚠️ ${b.needs_rate} lorry(ies) have no commission rate — enter the rate in the Commission Master first.`);
      onNeedRate?.(); return;
    }
    const msg = market
      ? `${b.owner_name} (fleet partner) — ${b.cycle_label}` + NL
        ...
        + `Approve and LOCK? Only TDS ${inr2(b.tds)} posts (deducted from the partner). The balance is paid separately with "💸 Pay balance".`
      : agency
      ? `${b.owner_name} — ${b.cycle_label}` + NL + `Payable to owner: ${inr2(b.payable)}` + NL + NL
        + `Approve and LOCK? ${inr2(b.payable)} is credited to the owner's ledger, commission ${inr2(b.commission)} is our income, TDS ${inr2(b.tds)}.`
      : `${b.owner_name} (own vehicle) — ${b.cycle_label}` + NL + NL
        + 'Approve and LOCK? Only manual adjustments post to the ledger — freight and HSD post through their own flows.';
    ...
    try {
      ...
      else setErr(e?.code === 'FORBIDDEN' ? 'Only an admin can approve.' : (e?.message ?? 'Approval failed'));
    }
    setBusy(false);
  };

  const reopen = async () => {
    const reason = window.prompt('Why modify? (a reason is required and stays in the audit trail)', '');
    if (!reason || reason.trim().length < 4) return;
    setBusy(true); setErr('');
    try {
      const j = await apiJson(`${api}/${id}/reopen`, { method: 'POST', body: JSON.stringify({ reason }) });
      alert('🔓 Bill reopened.' + (j.note ? String.fromCharCode(10) + j.note : ''));
  ...
  const whatsapp = async () => {
    try {
      ...
      alert(r?.via === 'server' ? '🟢 Sent.' : '📱 WhatsApp opened — send from there (the engine is offline).');
    } catch (e) { alert(`❌ ${e?.message ?? 'Failed'}`); }
  };

  const email = async () => {
    const to = window.prompt('Send to which e-mail address?', '');
    if (!to) return;
    setBusy(true);
    try {
      const j = await apiJson(`${api}/${id}/email`, { method: 'POST', body: JSON.stringify({ to }) });
      alert(`✉️ Sent — ${j.to}`);
    } catch (e) { alert(`❌ E-mail failed: ${e?.message ?? ''}`); }
    setBusy(false);
  };
  ...
  const printSheet = () => {
    ...
    if (market) {
      ...
      w.document.write(`<html><head><title>${esc(b.bill_no)} — ${esc(b.owner_name)}</title>
        ...
        <table class="foot"><tr><td>Total freight (${b.loads} loads)</td><td class="r">${inr2(b.partner_freight)}</td></tr>
        <tr><td>− Advance already paid</td><td class="r">${inr2(b.advances_paid)}</td></tr>
        <tr><td>− TDS 194C ${pct}%</td><td class="r">${b.tds === null ? '—' : inr2(b.tds)}</td></tr>
        ${n2(b.adj_income) ? `<tr><td>+ Other</td><td class="r">${inr2(b.adj_income)}</td></tr>` : ''}${n2(b.adj_expense) ? `<tr><td>− Deductions</td><td class="r">${inr2(b.adj_expense)}</td></tr>` : ''}
        <tr class="tot"><td>Balance</td><td class="r">${b.payable === null ? '—' : inr2(b.payable)}</td></tr></table>
        <div style="margin-top:10px;color:#555;font-size:10px">${b.pay_voucher_id ? 'Balance paid ' + inr2(b.paid_amount) + ' (' + day(b.paid_at) + ')' : b.status === 'APPROVED' ? 'Approved — payment pending' : esc(STATUS[b.status]?.t ?? b.status)}</div>
        </body></html>`);
    ...
    for (const v of lorries) {
      ...
      if (agency) {
        body += `<tr class="terms"><td colspan="9"></td><td colspan="7" class="r">Commission ${v.commission_basis === 'PCT' ? n2(v.commission_rate) + '%' : (v.commission_basis ?? '')} <b>${v.commission_amount === null ? 'rate missing' : inr2(v.commission_amount)}</b>
          &nbsp;·&nbsp; TDS ${n2(v.tds_pct)}% <b>${v.tds_amount === null ? '—' : inr2(v.tds_amount)}</b> &nbsp;·&nbsp; expenses recovered <b>${inr2(v.expenses_recovered)}</b> &nbsp;·&nbsp; payable to owner <b>${v.payable_to_owner === null ? '—' : inr2(v.payable_to_owner)}</b></td></tr>`;
      }
    }
    ...
    const foot = agency
      ? `<table class="foot"><tr><td>Freight (${b.trips} trip)</td><td class="r">${inr2(b.freight)}</td></tr>
         ${n2(b.adj_income) ? `<tr><td>+ Other income (manual)</td><td class="r">${inr2(b.adj_income)}</td></tr>` : ''}
         <tr><td>− Commission</td><td class="r">${b.commission === null ? 'rate missing' : inr2(b.commission)}</td></tr>
         <tr><td>− TDS 194C</td><td class="r">${b.tds === null ? '—' : inr2(b.tds)}</td></tr>
         <tr><td>− Expenses recovered (HSD, toll, advance, fooding, fixed, doc, other)</td><td class="r">${inr2(b.recovered)}</td></tr>
         ${n2(b.adj_expense) ? `<tr><td>− Manual deductions</td><td class="r">${inr2(b.adj_expense)}</td></tr>` : ''}
         <tr class="tot"><td>Payable to owner</td><td class="r">${b.payable === null ? '—' : inr2(b.payable)}</td></tr></table>`
      : `<table class="foot"><tr><td>Freight</td><td class="r">${inr2(b.freight)}</td></tr>
         <tr><td>− Total expenses</td><td class="r">${inr2(b.deductions)}</td></tr>
         <tr class="tot"><td>${n2(b.freight) + n2(b.adj_income) - n2(b.deductions) >= 0 ? 'Net margin' : 'Net loss'}</td><td class="r">${inr2(Math.abs(n2(b.freight) + n2(b.adj_income) - n2(b.deductions)))}</td></tr></table>`;
    w.document.write(`<html><head><title>${esc(b.bill_no)} — ${esc(b.owner_name)}</title>
      ...
      <table><thead><tr><th rowspan="2">Trip / Bill No</th><th class="k" colspan="8">EXPENSES</th><th class="i fold" colspan="7">BILL DETAILS</th></tr>
      <tr><th class="k">HSD</th><th class="k">Toll Tax</th><th class="k">Trip Fooding Alw.</th><th class="k">Trip Fixed Alw.</th><th class="k">Trip Advance</th><th class="k">Doc Exp</th><th class="k">Other Exp</th><th class="k">Total expenses</th>
      ...
      ${foot}
      <div class="note">* Rate = freight ÷ (KL × RTKM) where no rate is recorded on the trip. Shortage = the customer's penalty, for information. ${agency ? 'Trip Advance was paid on the owner\'s behalf and is therefore counted as an expense.' : 'On an own vehicle the advance stays on the driver\'s ledger.'}</div>
      </body></html>`);
    w.document.close(); w.focus(); w.print();
  };
  ...
  return (
    <div onClick={onClose}
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        {!b ? (
          <p style={{ color: C.mut, padding: '24px' }}>{err || 'Opening bill…'}</p>
        ) : (
          <>
            {/* ── header + toolbar ─────────────────────────────────── */}
            <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.line}`, display: 'flex', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap' }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: '21px', fontWeight: 800, color: C.ink, marginTop: '2px' }}>{b.owner_name}{!agency && <span style={{ color: C.mut, fontWeight: 500, fontSize: '14px' }}> · own vehicle (OWN)</span>}</div>
                ...
                <div style={{ marginTop: '7px', display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {n2(b.needs_rate) > 0 && (
                    <span onClick={() => onNeedRate?.()} style={{ color: C.crit, fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}>
                      ⚠️ {b.needs_rate} lorry(ies) without a commission rate — Commission Master →
                    </span>
                  )}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                {locked ? (
                  <button onClick={reopen} disabled={busy} style={btn('warn', !busy)} title="Admin only · reason required">🔓 Modify</button>
                ) : (
                  <>
                ...
                {!locked && (
                  <button onClick={approve} disabled={busy || dirty} style={btn('good', !busy && !dirty)}
                    title={dirty ? 'Save first' : market ? 'Admin only · posts TDS and locks' : 'Admin only · posts to the owner ledger and locks'}>
                    {market ? '✅ Approve (TDS + lock)' : '✅ Approve & Post'}
                  </button>
                )}
                {market && locked && !b.pay_voucher_id && (
                  <button onClick={payBalance} disabled={busy} style={btn('mkt', !busy)} title="Admin only · one PAYMENT voucher from the bank">💸 Pay balance</button>
                )}
                {market && b.pay_voucher_id && (
                  <span style={{ color: C.mkt, fontSize: '11.5px', fontWeight: 700, alignSelf: 'center' }}>💸 paid {inr2(b.paid_amount)} · {day(b.paid_at)}</span>
                )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px,1fr))', gap: '1px', background: C.line, borderBottom: `1px solid ${C.line}` }}>
              {market ? (<>
                ...
                {kpi('Partner freight (cost)', inr2(b.partner_freight), C.mkt)}
                ...
                {kpi(`TDS 194C${b.tds_pct !== null && b.tds_pct !== undefined ? ` ${n2(b.tds_pct)}%` : ''}`, b.tds === null ? 'rate missing' : inr2(b.tds), b.tds === null ? C.dim : C.ink)}
                {kpi('Payable to partner (balance)', b.payable === null ? 'after rate' : inr2(b.payable), b.payable === null ? C.dim : C.good)}
              </>) : (<>
              {kpi(`Freight (${b.trips} trip)`, inr2(b.freight), C.ink)}
              {kpi('Total expenses', inr2(b.deductions), C.crit)}
              {agency ? kpi('Commission', b.commission === null ? 'rate missing' : inr2(b.commission), b.commission === null ? C.dim : C.ink)
                      : kpi('Manual adj.', `${inr(b.adj_income)} / ${inr(b.adj_expense)}`, C.ai)}
              {agency ? kpi('TDS 194C', b.tds === null ? '—' : inr2(b.tds), b.tds === null ? C.dim : C.ink)
                      : kpi('Received', inr2(b.received), C.mut)}
              {agency ? kpi('Payable to owner', b.payable === null ? 'after rate' : inr2(b.payable), b.payable === null ? C.dim : C.good)
                      : kpi(netOwn >= 0 ? 'Net margin' : 'Net loss', inr2(Math.abs(netOwn)), netOwn >= 0 ? C.good : C.crit)}
              </>)}
            ...
            {editing && !locked && (
              <div style={{ margin: '12px 20px 0', padding: '9px 13px', border: '1px solid rgba(34,211,238,0.35)', background: 'rgba(34,211,238,0.06)', borderRadius: '9px', fontSize: '12px', color: C.ink2 }}>
                ✏️ Type an amount in a dashed box — it is recorded <b>under that trip</b> (fooding / fixed / doc / other). HSD comes from pump slips, Toll from FASTag, Advance from the driver ledger — those are not edited here.
                A manual adjustment (detention, bonus) can also be added under each lorry. Then <b>Save</b>.
              </div>
            )}
            ...
            <div style={{ margin: '12px 20px 0', overflowX: 'auto', border: `1px solid ${C.line}`, borderRadius: '10px', maxHeight: '62vh', overflowY: 'auto' }}>
              <table style={{ borderCollapse: 'collapse', minWidth: '1560px', width: '100%', fontSize: '12px' }}>
                <thead>
                  <tr>
                    <th rowSpan={2} style={{ ...th('n'), minWidth: '170px' }}>Trip / Bill No<br /><span style={{ letterSpacing: 0, fontWeight: 500, textTransform: 'none' }}>date · driver</span></th>
                    <th colSpan={8} style={{ ...th('k', 'center'), letterSpacing: '0.12em' }}>◀ EXPENSES — recorded under the trip ID</th>
                    <th colSpan={7} style={{ ...th('i', 'center'), ...fold, letterSpacing: '0.12em' }}>BILL DETAILS (IOCL format) ▶</th>
                  </tr>
                  <tr>
                    {['HSD', 'Toll Tax', 'Trip Fooding Alw.', 'Trip Fixed Alw.', 'Trip Advance', 'Doc Exp', 'Other Exp', 'Total expenses'].map((h) => (
                    ...
                    return (
                      <React.Fragment key={v.id}>
                        <tr onClick={() => setOpenLorry((o) => ({ ...o, [v.id]: !open }))} style={{ cursor: 'pointer' }}>
                          <td colSpan={16} style={{ ...td('n'), background: C.raised, color: C.ink, fontWeight: 700, borderTop: `1px solid ${C.line}` }}>
                            ...
                            <span style={{ fontWeight: 500, color: C.mut, fontSize: '11px', marginLeft: '8px' }}>
                              {v.fleet_class ?? 'not in master'} · {v.trips_count} trips · freight {inr(v.billed_amount)} · expenses {inr(v.bill_expense)}
                              {v.needs_rate && <span style={{ color: C.crit, marginLeft: '8px' }}>⚠️ rate missing</span>}
                              {v.stale && <span style={{ color: C.warn, marginLeft: '8px' }}>⚠️ trips changed — rebuild the draft</span>}
                              {n2(v.billed_amount) === 0 && <span style={{ color: C.crit, marginLeft: '8px' }}>freight ₹0 — billing pending</span>}
                            </span>
                          </td>
                        </tr>
                        {open && lines.map((l) => {
                          ...
                          return (
                            <tr key={l.trip_id}>
                              <td style={td('n')}>
                                ...
                                <div style={{ fontSize: '10.5px', color: C.dim }}>{l.iocl_bill_no ? l.trip_code + ' · ' : 'IOCL bill no pending · '}{dmy(l.loading_date)} → {dmy(l.unloading_date)}</div>
                                  ...
                                  <div style={{ fontSize: '10px', color: C.ai, marginTop: '2px' }}>
                                    {es.map((e) => (
                                      <div key={e.id}>{KINDS.find((k) => k[0] === e.kind)?.[1] ?? e.kind} {inr(e.amount)}{e.label ? ` · ${e.label}` : ''} <span style={{ color: C.dim }}>· {e.entered_by}</span>
                                        {editing && !locked && <span onClick={() => setRemoves((x) => [...x, e.id])} style={{ color: C.crit, cursor: 'pointer', marginLeft: '4px' }} title="Remove">×</span>}
                                      </div>
                                    ))}
                                  </div>
                                )}
                              ...
                              <td style={td('k', 'right')}>{agency ? money(l.advances) : <span style={{ color: C.dim }} title="Own vehicle: on the driver ledger, not an expense">{n2(l.advances) ? num2(l.advances) + ' ⓘ' : '0'}</span>}</td>
                              ...
                              <td style={{ ...td('i'), ...fold, whiteSpace: 'normal', minWidth: '180px' }}>
                                {l.dest || l.customer || <span style={{ color: C.dim }}>ship-to not recorded</span>}
                        ...
                        <tr>
                          ...
                          <td style={{ ...td('i'), ...fold, background: 'rgba(24,36,74,0.55)', color: C.dim }}>{v.trips_count} trip{n2(v.adj_income) > 0 && <div style={{ fontSize: '10px', color: C.ai }}>+ manual income {inr(v.adj_income)}</div>}</td>
                            ...
                            <td colSpan={7} style={{ ...td('i', 'right'), ...fold, background: 'rgba(24,36,74,0.3)', borderBottom: `2px solid ${C.line}`, fontSize: '11.5px', color: C.mut, whiteSpace: 'normal' }}>
                              {agency ? (
                                v.commission_amount === null
                                  ? <span style={{ color: C.crit }}>⚠️ commission rate not recorded — <span onClick={() => onNeedRate?.()} style={{ textDecoration: 'underline', cursor: 'pointer' }}>Commission Master</span></span>
                                  : <>Commission {v.commission_basis === 'PCT' ? `${n2(v.commission_rate)}%` : `${v.commission_basis} ${n2(v.commission_rate)}`} <b style={{ color: C.ink }}>{inr2(v.commission_amount)}</b>
                                     ...
                                     &nbsp;·&nbsp; expenses recovered <b style={{ color: C.ink }}>{inr2(v.expenses_recovered)}</b>
            ...
            <p style={{ color: C.dim, fontSize: '11px', margin: '8px 20px 0', lineHeight: 1.6 }}>
              * Rate = freight ÷ (KL × RTKM) where no rate is recorded on the trip. Shortage = the customer's penalty (information). HSD from pump slips, Toll from FASTag
              (crossings linked to the trip), Trip Advance from the driver ledger{agency ? ' — paid on the owner\'s behalf for an attached lorry, so counted as an expense' : ' — stays on the driver ledger for an own vehicle, not an expense'}.
            </p>
            </>)}

            {/* ── the foot: settlement + journal ────────────────────────── */}
            {market ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '14px', margin: '14px 20px 0' }}>
              <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '10px', padding: '14px 16px' }}>
                <h4 style={{ margin: '0 0 8px', fontSize: '12.5px', color: C.mut, letterSpacing: '.04em' }}>{agency ? 'SETTLEMENT — payable to the owner' : 'SETTLEMENT — own vehicle margin'}</h4>
                {[
                  ['Freight', `${b.trips} trip · billed amount`, b.freight, C.ink],
                  n2(b.adj_income) ? ['+ Other income', 'manual', b.adj_income, C.ai] : null,
                  agency ? ['− Commission', b.commission === null ? 'rate not recorded' : `${lorries.filter((v) => v.commission_basis === 'PCT').length ? 'PCT basis' : 'as per term'}`, b.commission, C.ink2] : null,
                  agency ? ['− TDS 194C', 'on (freight − commission)', b.tds, C.ink2] : null,
                  agency ? ['− Expenses recovered', `HSD ${inr(b.hsd)} · toll ${inr(b.toll)} · advance ${inr(b.advances)} · typed ${inr(n2(b.fooding) + n2(b.fixed_allowance) + n2(b.doc_expense) + n2(b.other_expense))}`, b.recovered, C.crit]
                         : ['− Total expenses', `HSD ${inr(b.hsd)} · toll ${inr(b.toll)} · typed ${inr(n2(b.fooding) + n2(b.fixed_allowance) + n2(b.doc_expense) + n2(b.other_expense))}`, b.expense_total, C.crit],
                  n2(b.adj_expense) ? ['− Manual deductions', 'added by reviewer', b.adj_expense, C.ai] : null,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', padding: '10px 0 0', marginTop: '4px', borderTop: `2px solid ${C.line}`, fontWeight: 800, fontSize: '15px' }}>
                  <span style={{ color: C.ink2 }}>{agency ? 'Payable to owner' : (netOwn >= 0 ? 'Net margin' : 'Net loss')}</span>
              ...
              <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '10px', padding: '14px 16px' }}>
                <h4 style={{ margin: '0 0 8px', fontSize: '12.5px', color: C.mut, letterSpacing: '.04em' }}>
                  {locked ? 'POSTED' : 'POSTS ON APPROVAL'} — books of {b.company_name ?? b.operating_company ?? 'company'}
                </h4>
                {(locked && Array.isArray(b.posted_lines) && b.posted_lines.length ? b.posted_lines : data?.journal?.lines ?? []).length === 0 ? (
                  <div style={{ color: C.dim, fontSize: '12.5px' }}>Nothing to post — {agency ? 'freight ₹0 or rate pending' : 'no manual adjustment'}. Approval only locks the bill.</div>
                ...
                <div style={{ color: C.dim, fontSize: '11px', marginTop: '8px', lineHeight: 1.6 }}>
                  Voucher ref <span style={{ fontFamily: 'monospace' }}>VEHBILL_{b.bill_no}</span> — posts once. After a Modify, re-approval posts only the difference as a new voucher (_R2); the original stays.
                </div>
              </div>
            </div>
            )}
            ...
            <div style={{ margin: '12px 20px 18px' }}>
              ...
              <div style={{ fontSize: '10.5px', color: C.dim, marginTop: '6px' }}>
                ...
                {locked && ' · After approval only Modify (admin, with reason), Print, WhatsApp, Email.'}
              </div>
            </div>
          </>
        )}
...
function MarketLoads({ b, loads, vendor, running, editing, billAdj, setBillAdj }) {
  ...
  const groups = [];
  for (const l of loads) {
    const key = l.truck ?? '(truck not recorded)';
    ...
    <>
      <div style={{ margin: '12px 20px 0', overflowX: 'auto', border: `1px solid ${C.line}`, borderRadius: '10px', maxHeight: '62vh', overflowY: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', minWidth: '1400px', width: '100%', fontSize: '12px' }}>
          <thead>
            <tr>
              <th rowSpan={2} style={{ ...th('n'), minWidth: '170px' }}>Load ID<br /><span style={{ letterSpacing: 0, fontWeight: 500, textTransform: 'none' }}>POD date · route</span></th>
              <th colSpan={4} style={{ ...th('c', 'center'), letterSpacing: '0.12em' }}>◀ CUSTOMER SIDE (our income · office only)</th>
            ...
            <tr>
              ...
              <th style={th('p', 'right')}>TDS{hasPct ? ` ${pct}%` : ''}</th><th style={th('p', 'right')}>Balance due</th><th style={th('p')}>POD</th><th style={th('p')}>Ledger</th>
            </tr>
          </thead>
          <tbody>
            {groups.map((g) => (
              <React.Fragment key={g.truck}>
                ...
                {g.rows.map((l) => (
                  <tr key={l.settlement_id ?? l.load_id}>
                    ...
                    <td style={td('c', 'right')}>{l.customer_rate === null || l.customer_rate === undefined ? <span style={{ color: C.warn }}>rate pending</span> : money(l.customer_rate, C.ink)}</td>
                    ...
                    <td style={td('p', 'right')}>{hasPct ? money(tdsOf(l)) : <span style={{ color: C.crit }}>rate missing</span>}</td>
            ...
            <tr>
              ...
              <td style={{ ...td('p', 'right'), background: C.raised, fontWeight: 800, borderTop: `2px solid ${C.line}` }}>{b.tds === null ? 'rate missing' : money(b.tds, C.ink, true)}</td>
      ...
      <p style={{ color: C.dim, fontSize: '11px', margin: '8px 20px 0', lineHeight: 1.6 }}>
        The partner pays its own diesel, toll and driver — hence no expense columns. TDS 194C once on the full partner freight, on this bill
        {vendor ? ` (${vendor.entity_type === 'FIRM' ? 'firm 2%' : vendor.entity_type === 'INDIVIDUAL' ? 'individual 1%' : vendor.tds_declaration_194c ? '194C(6) declaration — NIL' : vendor.pan_no ? 'individual/firm not chosen' : 'no PAN — 20%'})` : ''}.
        The customer side and the margin are never shown to the partner — Print / WhatsApp / Email carry the partner side only.
        {running && n2(running.n) > 0 && <> Loads in transit (POD pending, not on this bill): <b style={{ color: C.ink2 }}>{running.n} · {inr(running.partner_freight)}</b>.</>}
      </p>
      {(editing || billAdj.length > 0) && (
        <div style={{ margin: '10px 20px 0', padding: '10px 14px', border: `1px solid rgba(167,139,250,0.35)`, background: 'rgba(167,139,250,0.05)', borderRadius: '10px' }}>
          {editing ? (
            <AdjustmentEditor adj={billAdj} setAdj={setBillAdj} labels={{ income: '+ To partner', expense: '− Deductions' }} />
          ) : (
...
function MarketFoot({ b, journal, running, locked }) {
  const rows = [
    ...
    ['− Advance already paid', 'BZADV voucher, at loading', b.advances_paid, C.ink2],
    ...
    n2(b.adj_expense) ? ['− Deductions (manual)', 'damage, deposit, etc.', b.adj_expense, C.ai] : null,
  ].filter(Boolean);
  const lines = journal?.lines ?? [];
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '14px', margin: '14px 20px 0' }}>
      <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '10px', padding: '14px 16px' }}>
        <h4 style={{ margin: '0 0 8px', fontSize: '12.5px', color: C.mut, letterSpacing: '.04em' }}>SETTLEMENT — payable to the partner</h4>
        {rows.map((r) => (
          <div key={r[0]} style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', padding: '6px 0', borderBottom: '1px solid #1b2a4e', fontSize: '13px' }}>
            <span style={{ color: C.ink2 }}>{r[0]} <span style={{ color: C.dim, fontSize: '11px', marginLeft: '6px' }}>{r[1]}</span></span>
            <span style={{ color: r[2] === null ? C.dim : r[3], fontVariantNumeric: 'tabular-nums' }}>{r[2] === null ? '—' : inr2(r[2])}</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', padding: '10px 0 0', marginTop: '4px', borderTop: `2px solid ${C.line}`, fontWeight: 800, fontSize: '15px' }}>
          <span style={{ color: C.ink2 }}>Balance to pay</span>
        ...
        <div style={{ borderTop: `1px dashed ${C.line}`, marginTop: '8px', paddingTop: '6px' }}>
          {[
            ['Our margin', 'office only · income − cost, never a third entry', b.margin, C.good],
            ['Receivable from customer', 'Market Debtors · posted at POD', b.freight, C.ink2],
            running && n2(running.n) > 0 ? ['Loads in transit (POD pending)', 'not on this bill', running.partner_freight, C.dim] : null,
          ].filter(Boolean).map((r) => (
      ...
      <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '10px', padding: '14px 16px' }}>
        <h4 style={{ margin: '0 0 8px', fontSize: '12.5px', color: C.mut, letterSpacing: '.04em' }}>
          {locked ? 'POSTED' : 'POSTS ON APPROVAL'} — TDS only · books of {b.company_name ?? 'company'}
        </h4>
        {(locked && Array.isArray(b.posted_lines) && b.posted_lines.length ? b.posted_lines : lines).length === 0 ? (
          <div style={{ color: C.dim, fontSize: '12.5px' }}>TDS is nil or the rate is pending — approval only locks the bill.</div>
        ...
        <h4 style={{ margin: '12px 0 6px', fontSize: '12.5px', color: C.mut, letterSpacing: '.04em' }}>💸 ON PAYING THE BALANCE — one PAYMENT voucher</h4>
        {b.pay_voucher_id ? (
          <div style={{ fontSize: '12.5px', color: C.good }}>Paid {inr2(b.paid_amount)} · {day(b.paid_at)} · {b.paid_by} · all loads SETTLED · partner notified on WhatsApp.</div>
        ) : (
          <div style={{ fontSize: '12.5px', color: C.ink2 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '30px 1fr auto', gap: '8px', padding: '4px 0' }}><span style={{ fontFamily: 'monospace', color: C.dim, fontSize: '11px' }}>Cr</span><span>Bank / Cash <span style={{ color: C.dim, fontSize: '11px' }}>(chosen at payment)</span></span><span style={{ fontVariantNumeric: 'tabular-nums' }}>{b.payable === null ? '—' : num2(b.payable)}</span></div>
          </div>
        )}
        <div style={{ color: C.dim, fontSize: '11px', marginTop: '8px', lineHeight: 1.6 }}>
          Already posted: BZLOCK (cost) at award, BZINC (income) at POD, BZADV advance at loading — the bill never touches these again.
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
      {adj.map((a, i) => (
        <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '11.5px' }}>
          <span style={{ color: a.side === 'INCOME' ? C.good : C.crit, fontWeight: 700, minWidth: '54px' }}>{a.side === 'INCOME' ? (labels?.income ?? '+ INCOME') : (labels?.expense ?? '− EXPENSE')}</span>
      ...
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select value={side} onChange={(e) => setSide(e.target.value)} style={inp}>
          <option value="EXPENSE">{labels?.expense ?? '− Expense'}</option><option value="INCOME">{labels?.income ?? '+ Income'}</option>
        </select>
