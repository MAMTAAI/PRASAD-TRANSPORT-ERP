...
const GoogleTools = () => (
  <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginLeft: '15px' }}>
    <a href="https://mail.google.com/mail/u/0/#inbox" target="_blank" rel="noreferrer" title="Gmail"
       ...
       onMouseOver={(e) => e.currentTarget.style.borderColor = '#ea580c'} onMouseOut={(e) => e.currentTarget.style.borderColor = '#334155'}>
    ...
    <a href="https://calendar.google.com/calendar/u/0/r" target="_blank" rel="noreferrer" title="Calendar"
       style={{ background: '#1e293b', border: '1px solid #334155', padding: '8px', borderRadius: '8px', display: 'flex', transition: '0.3s' }}
       onMouseOver={(e) => e.currentTarget.style.borderColor = '#38bdf8'} onMouseOut={(e) => e.currentTarget.style.borderColor = '#334155'}>
    ...
    <a href="https://drive.google.com/drive/u/0/my-drive" target="_blank" rel="noreferrer" title="Google Drive"
       style={{ background: '#1e293b', border: '1px solid #334155', padding: '8px', borderRadius: '8px', display: 'flex', transition: '0.3s' }}
       onMouseOver={(e) => e.currentTarget.style.borderColor = '#10b981'} onMouseOut={(e) => e.currentTarget.style.borderColor = '#334155'}>
...
export default function Dashboard({ activeModule }: DashboardProps) {
  ...
  if (loading) {
    return <div style={{ color: '#38bdf8', padding: '50px', textAlign: 'center', fontSize: '22px', fontWeight: '900', letterSpacing: '1px' }}>⏳ Compiling Live Analytical Dashboard...</div>;
  }
  ...
  const expiringDrivers = drivers.filter(d => {
    const status = getExpiryStatus(d.dl_expiry_date || d.dl_validity || d.expiry_date || d.license_expiry);
    return status === 'EXPIRED' || status === 'EXPIRING_SOON';
  }).map(d => ({
    name: d.name || d.driver_name, mobile: d.mobile_no || d.mobile || d.contact,
    date: d.dl_expiry_date || d.dl_validity || d.expiry_date || d.license_expiry,
    status: getExpiryStatus(d.dl_expiry_date || d.dl_validity || d.expiry_date || d.license_expiry)
  }));
  ...
  const barChartData = [
    ...
    { month: 'Mar', inc: 120 * filterMultiplier, exp: 40 * filterMultiplier }, // Boosted March for impact!
  ];
    ...
    return (
      <div style={{ color: 'white', fontFamily: "'Inter', sans-serif", paddingBottom: '50px' }}>
        <style>{`
          ...
          .kpi-card::after { content: '👆 Click to view detailed list'; position: absolute; bottom: 12px; right: 20px; font-size: 11px; opacity: 0; color: rgba(255,255,255,0.9); transition: 0.3s; font-weight: bold; background: rgba(0,0,0,0.5); padding: 4px 8px; border-radius: 10px; }
          .kpi-card:hover::after { opacity: 1; right: 15px; }
          .modal-table th { background: rgba(255,255,255,0.05); padding: 15px; text-align: left; color: #94a3b8; font-size: 12px; text-transform: uppercase; border-bottom: 2px solid #334155; letter-spacing: 1px; }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '36px', color: '#fff', display: 'flex', alignItems: 'center', gap: '12px', fontWeight: '900', letterSpacing: '-1px' }}>
              🚛 Master Fleet Command
            </h2>
            <p style={{ margin: '5px 0 0 0', color: '#38bdf8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '2px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            <GoogleTools />
            <button onClick={() => window.location.reload()} style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: '#38bdf8', border: '1px solid #38bdf8', padding: '10px 20px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: '0.3s', boxShadow: '0 0 15px rgba(56, 189, 248, 0.2)' }} onMouseOver={e=>e.currentTarget.style.transform='scale(1.05)'} onMouseOut={e=>e.currentTarget.style.transform='scale(1)'}>
              🔄 Sync Live Radar
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '25px', marginBottom: '35px' }}>
          <div className="kpi-card" onClick={() => setDetailModal('FLEET')} style={{ background: 'linear-gradient(135deg, rgba(56, 189, 248, 0.15), rgba(37, 99, 235, 0.15))', border: '1px solid rgba(56,189,248,0.4)', padding: '25px', borderRadius: '15px' }}>
            <div style={{ color: '#38bdf8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Total Fleet Strength</div>
            <div style={{ fontSize: '42px', fontWeight: '900', color: '#fff', marginTop: '10px' }}>{vehicles.length || 0}</div>
          </div>
          <div className="kpi-card" onClick={() => setDetailModal('LOADING')} style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.15), rgba(217, 119, 6, 0.15))', border: '1px solid rgba(245,158,11,0.4)', padding: '25px', borderRadius: '15px' }}>
            <div style={{ color: '#f59e0b', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Pending For Loading</div>
            <div style={{ fontSize: '42px', fontWeight: '900', color: '#fff', marginTop: '10px' }}>{pendingLoadingTrips.length} <span style={{fontSize:'24px'}}>📦</span></div>
          </div>
          <div className="kpi-card" onClick={() => setDetailModal('TRANSIT')} style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15), rgba(5, 150, 105, 0.15))', border: '1px solid rgba(16,185,129,0.4)', padding: '25px', borderRadius: '15px' }}>
            <div style={{ color: '#10b981', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Active Trips (In Transit)</div>
            <div style={{ fontSize: '42px', fontWeight: '900', color: '#fff', marginTop: '10px' }}>{activeTransitTrips.length} <span style={{fontSize:'24px'}}>🟢</span></div>
          </div>
          <div className="kpi-card" onClick={() => setDetailModal('UNLOADING')} style={{ background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.15), rgba(220, 38, 38, 0.15))', border: '1px solid rgba(239,68,68,0.4)', padding: '25px', borderRadius: '15px' }}>
            <div style={{ color: '#ef4444', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Reached (Pending Unload)</div>
            <div style={{ fontSize: '42px', fontWeight: '900', color: '#fff', marginTop: '10px' }}>{pendingUnloadingTrips.length} <span style={{fontSize:'24px'}}>⏳</span></div>
          </div>
        </div>
        ...
        {/* AI INSIGHTS */}
        <div style={{ background: 'linear-gradient(135deg, rgba(139, 92, 246, 0.15), rgba(56, 189, 248, 0.15))', border: '1px solid rgba(139, 92, 246, 0.4)', borderRadius: '15px', padding: '30px', display: 'flex', alignItems: 'center', gap: '25px', boxShadow: '0 10px 30px rgba(139, 92, 246, 0.2)' }}>
          <div style={{ fontSize: '50px', filter: 'drop-shadow(0 0 15px rgba(192,132,252,0.8))' }}>🤖</div>
          <div>
            <h3 style={{ margin: '0 0 10px 0', color: '#c084fc', fontSize: '22px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: '900' }}>
              ...
              <span style={{ background: '#c084fc', color: '#0f172a', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '900', letterSpacing: '1px' }}>LIVE SYNC</span>
            </h3>
            <p style={{ margin: 0, color: '#e2e8f0', fontSize: '15px', lineHeight: '1.6' }}>
              {expiringVehicles.length > 0 ? <span style={{color: '#f59e0b'}}>⚠️ Attention! <b>{expiringVehicles.length} vehicles</b> have critical documents expiring within 15 days. </span> : <span style={{color: '#10b981'}}>✅ All fleet documents are up to date! </span>}
              <br/>
              {upcomingMaintenance.length > 0 ? <span style={{color: '#38bdf8'}}>🛠️ Please check workshop! <b>{upcomingMaintenance.length} maintenance schedules</b> are pending. </span> : ''}
              <br/>
              {pendingUnloadingTrips.length > 0 ? <span style={{color: '#ef4444'}}>⏳ Currently, <b>{pendingUnloadingTrips.length} vehicles</b> are waiting for unloading. Suggest prioritizing unloads to free up fleet.</span> : `🚀 Operations running smoothly at maximum efficiency.`}
            </p>
          </div>
        </div>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(2, 6, 23, 0.95)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
            <div style={{ background: '#0f172a', width: '100%', maxWidth: '1000px', maxHeight: '85vh', borderRadius: '15px', border: '1px solid #38bdf8', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px rgba(0,0,0,0.8)' }}>
              <div style={{ padding: '20px 30px', borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#1e293b', borderRadius: '15px 15px 0 0' }}>
                ...
                <button onClick={() => setDetailModal(null)} style={{ background: 'transparent', border: 'none', color: '#ef4444', fontSize: '28px', cursor: 'pointer', transition: '0.2s' }} onMouseOver={e=>e.currentTarget.style.transform='scale(1.2)'} onMouseOut={e=>e.currentTarget.style.transform='scale(1)'}>✕</button>
              </div>
  ...
  return (
    <div style={{ color: 'white', fontFamily: "'Inter', sans-serif", paddingBottom: '50px' }}>
      <style>{`
        ...
        /* 📊 SMART GRAPHICS CSS FOR BAR CHART */
        .bar-group { position: relative; display: flex; gap: 4px; align-items: flex-end; height: 100%; width: 100%; justify-content: center; padding-bottom: 5px; }
        .bar { width: 18px; border-radius: 4px 4px 0 0; transition: all 0.5s ease; position: relative; cursor: pointer; }
        .bar:hover::before { content: attr(data-val); position: absolute; top: -30px; left: 50%; transform: translateX(-50%); background: #fff; color: #000; padding: 4px 8px; border-radius: 4px; font-size: 11px; font-weight: bold; white-space: nowrap; z-index: 10; box-shadow: 0 4px 10px rgba(0,0,0,0.5); }
        .bar.inc { background: linear-gradient(to top, #059669, #34d399); box-shadow: 0 0 10px rgba(16,185,129,0.2); }
        .bar.inc:hover { box-shadow: 0 0 20px rgba(16,185,129,0.8); filter: brightness(1.2); }
        .bar.exp { background: linear-gradient(to top, #dc2626, #f87171); box-shadow: 0 0 10px rgba(239,68,68,0.2); }
        .bar.exp:hover { box-shadow: 0 0 20px rgba(239,68,68,0.8); filter: brightness(1.2); }
      `}</style>

      {/* --- FINANCE HEADER WITH GOOGLE TOOLS --- */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '36px', color: '#fff', display: 'flex', alignItems: 'center', gap: '12px', fontWeight: '900', letterSpacing: '-1px' }}>💰 Master Finance Hub</h2>
          <p style={{ margin: '5px 0 0 0', color: '#10b981', fontSize: '13px', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase' }}>Real-time Consolidated Financial & Cashflow Overview</p>
        </div>
        
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            <GoogleTools />
            <button onClick={() => alert("Report downloaded successfully!")} style={{ background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: '0.3s', boxShadow: '0 5px 15px rgba(16, 185, 129, 0.4)' }} onMouseOver={e=>e.currentTarget.style.transform='scale(1.05)'} onMouseOut={e=>e.currentTarget.style.transform='scale(1)'}>
              📥 Download MIS Report
            </button>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(30, 41, 59, 0.5)', border: '1px solid rgba(255,255,255,0.05)', padding: '20px', borderRadius: '15px', marginBottom: '25px', display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '250px' }}>
          <label style={{ color: '#38bdf8', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Company Level Filter</label>
        ...
        <div style={{ flex: 1, minWidth: '250px' }}>
          <label style={{ color: '#94a3b8', fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Branch Level Filter</label>
      ...
      {/* Financial Cards (WITH SMART TREND INDICATORS) */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '25px', marginBottom: '35px' }}>
        <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', borderLeft: '5px solid #10b981', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Total Revenue (YTD)</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '38px', fontWeight: '900', color: '#10b981', marginTop: '5px' }}>{formatCurrency(totalRevenue)}</div>
            <div style={{ background: 'rgba(16,185,129,0.2)', color: '#10b981', padding: '5px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>+14.2% ↗</div>
          </div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', borderLeft: '5px solid #ef4444', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Total Expenses</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '38px', fontWeight: '900', color: '#ef4444', marginTop: '5px' }}>{formatCurrency(totalExpenses)}</div>
            <div style={{ background: 'rgba(239,68,68,0.2)', color: '#ef4444', padding: '5px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>+5.1% ↗</div>
          </div>
        </div>
        <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', borderLeft: '5px solid #3b82f6', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Net Profit</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: '38px', fontWeight: '900', color: '#38bdf8', marginTop: '5px' }}>{formatCurrency(netProfit)}</div>
            <div style={{ background: 'rgba(56,189,248,0.2)', color: '#38bdf8', padding: '5px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>+22.8% 🚀</div>
          </div>
        </div>

        <div className="kpi-card" onClick={() => setDetailModal('DEBTORS')} style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.15), #1e293b)', borderLeft: '5px solid #f59e0b', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(245,158,11,0.3)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Accounts Receivable (Debtors)</div>
          <div style={{ fontSize: '38px', fontWeight: '900', color: '#f59e0b', marginTop: '5px' }}>{formatCurrency(receivable)}</div>
          <div style={{ color: '#fcd34d', fontSize: '12px', marginTop: '8px', fontWeight: 'bold' }}>Pending payments from Customers</div>
        </div>

        <div className="kpi-card" onClick={() => setDetailModal('CREDITORS')} style={{ background: 'linear-gradient(135deg, rgba(236,72,153,0.15), #1e293b)', borderLeft: '5px solid #ec4899', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(236,72,153,0.3)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Accounts Payable (Creditors)</div>
          <div style={{ fontSize: '38px', fontWeight: '900', color: '#ec4899', marginTop: '5px' }}>{formatCurrency(payable)}</div>
          <div style={{ color: '#fbcfe8', fontSize: '12px', marginTop: '8px', fontWeight: 'bold' }}>Dues for Vendors & Pumps</div>
        </div>

        <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', borderLeft: '5px solid #8b5cf6', padding: '25px', borderRadius: '15px', boxShadow: '0 15px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ color: '#94a3b8', fontSize: '13px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' }}>Liquid Bank Balances</div>
          <div style={{ fontSize: '38px', fontWeight: '900', color: '#c084fc', marginTop: '5px' }}>{formatCurrency(bankBalance)}</div>
          <div style={{ color: '#d8b4fe', fontSize: '12px', marginTop: '8px', fontWeight: 'bold' }}>Cash in Hand & Bank</div>
        </div>
      </div>

      {/* 📈 SMART GRAPHICS BAR CHART */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '30px', marginBottom: '30px' }}>
        <div style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '15px', padding: '30px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
          <h3 style={{ margin: '0 0 20px 0', color: '#fff', fontSize: '20px', display: 'flex', justifyContent: 'space-between' }}>
            <span>📉 6-Month Trend (Income vs Expense)</span>
            <span style={{ fontSize: '12px', color: '#94a3b8', fontWeight: 'normal' }}>*Values in Lakhs</span>
          </h3>
          
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-around', height: '280px', borderBottom: '2px solid #475569', paddingBottom: '10px', gap: '15px', marginTop: '30px' }}>
            {barChartData.map((data, index) => (
              <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '60px', gap: '10px', height: '100%' }}>
                <div className="bar-group">
                  <div className="bar inc" style={{ height: getBarHeight(data.inc) }} data-val={`₹${data.inc.toFixed(0)}L`}></div>
                  <div className="bar exp" style={{ height: getBarHeight(data.exp) }} data-val={`₹${data.exp.toFixed(0)}L`}></div>
                </div>
                <span style={{ color: '#cbd5e1', fontSize: '13px', fontWeight: 'bold', letterSpacing: '1px' }}>{data.month}</span>
              </div>
            ))}
          </div>
          
          <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', marginTop: '25px' }}>
             <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: '#94a3b8', fontWeight: 'bold' }}>
               <div style={{ width: '12px', height: '12px', background: '#10b981', borderRadius: '3px' }}></div> Income
             </div>
             <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: '#94a3b8', fontWeight: 'bold' }}>
               <div style={{ width: '12px', height: '12px', background: '#ef4444', borderRadius: '3px' }}></div> Expense
             </div>
          </div>
        </div>
      </div>

      {/* 🔍 DRILL-DOWN MODALS FOR FINANCE */}
      {detailModal && (detailModal === 'DEBTORS' || detailModal === 'CREDITORS') && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(2, 6, 23, 0.95)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
          <div style={{ background: '#0f172a', width: '100%', maxWidth: '1000px', maxHeight: '85vh', borderRadius: '15px', border: `1px solid ${detailModal === 'DEBTORS' ? '#f59e0b' : '#ec4899'}`, display: 'flex', flexDirection: 'column', boxShadow: '0 30px 60px rgba(0,0,0,0.8)' }}>
            <div style={{ padding: '20px 30px', borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#1e293b', borderRadius: '15px 15px 0 0' }}>
              <h3 style={{ margin: 0, color: detailModal === 'DEBTORS' ? '#f59e0b' : '#ec4899', fontSize: '24px' }}>
                ...
                {detailModal === 'DEBTORS' && (
                  <>
                    ...
                    <tbody>
                      {debtorsList.map((d, i) => (
                        <tr key={i}>
                          <td style={{ color: '#fff', fontWeight: 'bold', fontSize: '15px' }}>{d.name}</td>
                          <td style={{ color: '#f59e0b', fontWeight: '900', fontSize: '18px' }}>₹ {d.due.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                          <td><span style={{ background: d.status.includes('Overdue') ? 'rgba(239,68,68,0.2)' : 'rgba(245,158,11,0.2)', color: d.status.includes('Overdue') ? '#ef4444' : '#f59e0b', padding: '6px 15px', borderRadius: '20px', fontSize: '11px', fontWeight: '900', letterSpacing: '1px' }}>{d.status}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </>
                  ...
                  <>
                    ...
                    <tbody>
                      {creditorsList.map((c, i) => (
                        <tr key={i}>
                          <td style={{ color: '#fff', fontWeight: 'bold', fontSize: '15px' }}>{c.name}</td>
                          <td style={{ color: '#94a3b8' }}>{c.type}</td>
                          <td style={{ color: '#ec4899', fontWeight: '900', fontSize: '18px' }}>₹ {c.due.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                          <td><span style={{ background: c.status === 'Urgent' ? 'rgba(239,68,68,0.2)' : 'rgba(16,185,129,0.2)', color: c.status === 'Urgent' ? '#ef4444' : '#10b981', padding: '6px 15px', borderRadius: '20px', fontSize: '11px', fontWeight: '900', letterSpacing: '1px' }}>{c.status}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </>
