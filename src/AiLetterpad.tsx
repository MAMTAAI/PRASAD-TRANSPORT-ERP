...
export default function AiLetterPad() {
  ...
  return (
    <div style={{ padding: '30px', minHeight: '100vh', background: '#0a1024', fontFamily: "'Inter', sans-serif" }}>
      <style>{`
        .clean-card { background: #121c38; border: 1px solid #18244a; border-radius: 12px; box-shadow: 0 10px 20px rgba(0,0,0,0.5); }
        ...
        .btn-success { background: linear-gradient(135deg, #2fe39b, #2fe39b); color: white; border: none; padding: 12px 15px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; font-size: 13px; }
        .btn-success:hover { transform: translateY(-2px); box-shadow: 0 4px 15px rgba(47, 227, 155, 0.4); }
        .btn-outline { background: transparent; color: #c4d1ea; border: 1px solid #3d548a; padding: 12px 15px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; font-size: 13px; display: flex; align-items: center; justify-content: center; gap: 8px;}
        .btn-outline:hover { background: #18244a; color: white; }
        
        .clean-input { background: #18244a; border: 1px solid #27395f; border-radius: 8px; color: white; padding: 12px; width: 100%; box-sizing: border-box; font-size: 13px; outline: none; transition: 0.3s;}
        .clean-input:focus { border-color: #22d3ee; background: #121c38; }
        ...
        .ql-toolbar { background: #eef3fa; border-radius: 8px 8px 0 0; border: 1px solid #c4d1ea !important; border-bottom: none !important; }
        .ql-editor { min-height: 500px; padding: 40px; border: 1px solid #c4d1ea; box-shadow: inset 0 0 10px rgba(0,0,0,0.05); }
      `}</style>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', borderBottom: '1px solid #18244a', paddingBottom: '15px' }}>
        <div>
          <h1 style={{ margin: 0, color: '#f6f8fd', fontSize: '32px', display: 'flex', alignItems: 'center', gap: '10px', fontWeight: '900' }}>
            <span style={{ fontSize: '36px', filter: 'drop-shadow(0 0 10px rgba(167, 139, 250,0.8))' }}>📝</span> Document & Letter Pad
          </h1>
          <p style={{ color: '#9aadd4', margin: '5px 0 0 0', fontSize: '14px' }}>Create, edit, print, and save official letters manually or using <b style={{color: '#a78bfa'}}>Mamta AI</b>.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '25px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* SETUP FORM */}
          <div className="clean-card" style={{ padding: '25px', borderTop: '4px solid #22d3ee' }}>
            <div style={{ marginBottom: '15px' }}>
              <label style={{ fontSize:'12px', color:'#22d3ee', fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>1. Select Company Letterhead *</label>
              <select className="clean-input" style={{ border: '1px solid #22d3ee', fontWeight: 'bold', color: '#22d3ee' }} value={letterhead} onChange={e=>setLetterhead(e.target.value)}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
              <div>
                <label style={{ fontSize:'12px', color:'#9aadd4', fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>2. To (Authority / Company Name)</label>
                <input 
              ...
              <div>
                <label style={{ fontSize:'12px', color:'#9aadd4', fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>3. Subject / Action Needed</label>
                <input 
              ...
              <div>
                <label style={{ fontSize:'12px', color:'#9aadd4', display: 'block', marginBottom: '5px' }}>Attach Vehicle (Optional)</label>
              ...
              <div>
                <label style={{ fontSize:'12px', color:'#9aadd4', display: 'block', marginBottom: '5px' }}>Attach Driver (Optional)</label>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
              <button className="btn-outline" style={{ flex: 1, borderColor: '#3b82f6', color: '#22d3ee' }} onClick={handleBlankDocument}>
                📄 Start Blank Letter (Manual)
              </button>
              <button className="btn-primary" style={{ flex: 1, background: 'linear-gradient(135deg, #a78bfa, #9333ea)', boxShadow: '0 4px 15px rgba(167, 139, 250, 0.4)' }} onClick={handleMamtaAiDraft}>
          ...
          {/* 📝 EDITOR AREA */}
          <div className="clean-card" style={{ padding: '25px', borderTop: '4px solid #2fe39b' }}>
            {isAiThinking ? (
              <div style={{ textAlign: 'center', padding: '100px 20px', color: '#a78bfa' }}>
                <div style={{ fontSize: '50px', marginBottom: '15px', animation: 'bounce 2s infinite' }}>🧠</div>
                <h3 style={{ margin: 0 }}>Mamta AI is Drafting...</h3>
                <p style={{ color: '#9aadd4', fontSize: '13px' }}>Analyzing authority and generating professional legal text.</p>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '15px' }}>
                  <div style={{ flex: 1, marginRight: '15px' }}>
                    <label style={{ fontSize:'12px', color:'#2fe39b', fontWeight: 'bold', display: 'block', marginBottom: '5px' }}>File Name (For Saving) *</label>
                    <input className="clean-input" placeholder="e.g. IOCL Driver Auth - AS01C1234" value={docTitle} onChange={e=>setDocTitle(e.target.value)} style={{ border: '1px solid #2fe39b' }} />
                  </div>
                ...
                <div style={{ border: '2px solid #27395f', borderRadius: '8px', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
                  ...
                  <button className="btn-outline" style={{ flex: 1, borderColor: '#ff6b81', color: '#ff6b81' }} onClick={() => { setEditorContent(''); setDocTitle(''); setEditingDocId(null); }}>
                    🗑️ Clear Editor
                  </button>
                </div>
              </>
        ...
        <div className="clean-card" style={{ padding: '25px', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)', borderTop: '4px solid #ffb224' }}>
          <div style={{ borderBottom: '1px solid #18244a', paddingBottom: '15px', marginBottom: '15px' }}>
            <h3 style={{ color: '#ffb224', margin: '0 0 15px 0', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              📁 Saved Documents Lib
            </h3>
            {/* 🔍 SEARCH BOX */}
          ...
          <div style={{ flex: 1, overflowY: 'auto', paddingRight: '5px' }}>
            {loading ? <p style={{ color: '#9aadd4', fontSize: '13px', textAlign: 'center', marginTop: '20px' }}>Loading library...</p> : filteredDocs.length === 0 ? <p style={{ color: '#5d7196', fontSize: '13px', textAlign: 'center', marginTop: '20px' }}>No documents found.</p> : (
              filteredDocs.map((doc, i) => (
                <div key={i} style={{ background: '#18244a', padding: '15px', borderRadius: '10px', marginBottom: '12px', position: 'relative', borderLeft: '3px solid #22d3ee', transition: '0.2s' }} onMouseOver={e=>e.currentTarget.style.transform='translateX(5px)'} onMouseOut={e=>e.currentTarget.style.transform='translateX(0)'}>
                  
                  {/* 🗑️ DELETE ICON */}
                  <button 
                    onClick={() => handleDelete(doc.id, doc.title)}
                    style={{ position: 'absolute', top: '10px', right: '10px', background: 'transparent', border: 'none', color: '#ff6b81', cursor: 'pointer', fontSize: '16px', fontWeight: 'bold' }}
                    title="Delete Document"
                  >✕</button>

                  <h4 style={{ margin: '0 0 5px 0', color: '#fff', fontSize: '15px', paddingRight: '25px', lineHeight: '1.3' }}>{doc.title}</h4>
                  <p style={{ margin: '0 0 12px 0', fontSize: '11px', color: '#9aadd4' }}>
                    📅 {new Date(doc.createdAt).toLocaleDateString('en-IN')} | 🏢 {doc.authority || 'General'}
                  </p>
                  
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <button style={{ flex: 1, background: '#27395f', color: '#22d3ee', border: '1px solid #3d548a', padding: '8px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', transition: '0.2s' }} onMouseOver={e=>e.currentTarget.style.background='#3d548a'} onMouseOut={e=>e.currentTarget.style.background='#27395f'} onClick={() => {
                    ...
                    <button style={{ flex: 1, background: 'rgba(255, 178, 36, 0.1)', color: '#ffb224', border: '1px solid #ffb224', padding: '8px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', transition: '0.2s' }} onMouseOver={e=>{e.currentTarget.style.background='#f59e0b'; e.currentTarget.style.color='#fff';}} onMouseOut={e=>{e.currentTarget.style.background='rgba(255, 178, 36, 0.1)'; e.currentTarget.style.color='#ffb224';}} onClick={() => handlePrint(doc.content)}>
                      🖨️ Quick Print
                    </button>
                  </div>
                </div>
