    ...
    <div className="glass-card" style={{ padding: '18px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '14px', flexWrap: 'wrap', alignItems: 'flex-start', marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <span style={{ width: '6px' }} />
          {[['', 'Sab lorry'], ['AGENCY', 'Attached / market'], ['OWN', 'Apni gaadi']].map((s) => (
      ...
      <div style={{ overflowX: 'auto', border: `1px solid ${C.line}`, borderRadius: '10px' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', minWidth: '1080px', borderCollapse: 'collapse', fontSize: '12.5px' }}>
            <thead>
              <tr>
                <th style={th}>Bill No</th><th style={th}>Malik (owner)</th><th style={th}>Cycle</th>
                <th style={{ ...th, textAlign: 'right' }}>Lorry</th><th style={{ ...th, textAlign: 'right' }}>Trip</th>
                <th style={{ ...th, textAlign: 'right' }}>Freight</th><th style={{ ...th, textAlign: 'right' }}>Kul kharch</th>
                <th style={{ ...th, textAlign: 'right' }}>Commission</th><th style={{ ...th, textAlign: 'right' }}>TDS</th>
            ...
            <tbody>
              {pg.slice.map((b) => {
                ...
                return (
                  <tr key={b.id} onClick={() => setOpenId(b.id)} style={{ cursor: 'pointer' }} title="Bill kholne ke liye click karein">
                    <td style={{ ...td, fontFamily: 'monospace', color: C.cyan }}>{b.bill_no}</td>
                    ...
                    <td style={{ ...tdR, color: C.crit }}>{inr2(b.deductions)}</td>
                    <td style={{ ...tdR, color: b.commission === null ? C.dim : C.ink2 }}>{agency ? (b.commission === null ? '—' : inr2(b.commission)) : '—'}</td>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                {!locked && (
                  <button onClick={approve} disabled={busy || dirty} style={btn('good', !busy && !dirty)}
                    title={dirty ? 'Pehle Save kijiye' : 'Sirf admin · owner ke khaate me post + lock'}>✅ Approve &amp; Post</button>
                )}
...
// ══ MANUAL ADJUSTMENT, PER LORRY ═══════════════════════════════════════════
function AdjustmentEditor({ adj, setAdj }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
      {adj.map((a, i) => (
        <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '11.5px' }}>
          <span style={{ color: a.side === 'INCOME' ? C.good : C.crit, fontWeight: 700, minWidth: '54px' }}>{a.side === 'INCOME' ? '+ AAY' : '− KHARCH'}</span>
      ...
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select value={side} onChange={(e) => setSide(e.target.value)} style={inp}>
          <option value="EXPENSE">− Kharch</option><option value="INCOME">+ Aay</option>
        </select>
