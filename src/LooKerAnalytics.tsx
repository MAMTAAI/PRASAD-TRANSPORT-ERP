...
export default function LookerAnalytics() {
  ...
  return (
    <div style={{ 
      ...
      minHeight: '100vh', 
      background: 'radial-gradient(circle at top right, #121c38, #0a1024)',
      ...
      <style>{`
        ...
        .glow-btn { background: linear-gradient(135deg, #8b5cf6, #7c8cff); color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; font-size: 14px; box-shadow: 0 4px 15px rgba(124, 140, 255, 0.4); display: flex; align-items: center; gap: 8px;}
        .glow-btn:hover { box-shadow: 0 4px 25px rgba(124, 140, 255, 0.7); transform: scale(1.05); }
        .iframe-container { width: 100%; height: ${isFullscreen ? '100vh' : '75vh'}; border-radius: ${isFullscreen ? '0' : '15px'}; overflow: hidden; border: ${isFullscreen ? 'none' : '1px solid #27395f'}; }
      `}</style>
      ...
      {!isFullscreen && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
          <div>
            ...
            <p style={{ color: '#9aadd4', margin: '5px 0', fontSize: '14px' }}>Powered by Looker Studio Pro AI Analytics</p>
          </div>
          <div style={{ display: 'flex', gap: '15px' }}>
            <button className="glow-btn" onClick={toggleFullscreen} style={{ background: '#121c38', border: '1px solid #22d3ee', color: '#22d3ee', boxShadow: 'none' }}>
              🔲 Full Screen Mode
            </button>
      ...
      <div className={`glass-card iframe-container`} style={{ padding: isFullscreen ? '0' : '10px' }}>
        ...
        {isFullscreen && (
          <button onClick={toggleFullscreen} style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 1000, background: 'rgba(255, 107, 129,0.9)', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '50px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 5px 15px rgba(0,0,0,0.5)' }}>
            ✕ Exit Fullscreen
          </button>
        )}
      ...
      {!isFullscreen && (
        <div style={{ marginTop: '20px', textAlign: 'center', color: '#5d7196', fontSize: '12px' }}>
