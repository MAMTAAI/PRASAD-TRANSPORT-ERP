  ...
  return (
    <div style={{ padding: '20px 30px', minHeight: '100vh', background: '#0a1024', color: 'white', fontFamily: "'Inter', sans-serif" }}>
      ...
      <style>{`
        ...
        input[type="checkbox"] { cursor: pointer; accent-color: #22d3ee; width: 16px; height: 16px; }
        .module-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 0; border-bottom: 1px dotted #18244a; font-size: 13px; }
      `}</style>

      {/* HEADER */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px', color: '#2fe39b' }}>🏢 BRANCH MASTER & SETUP</h1>
          <p style={{ color: '#9aadd4', margin: '5px 0', fontSize: '14px' }}>Control which modules are available for each branch</p>
        </div>
      ...
      <div style={{ background: 'rgba(24, 36, 74, 0.5)', padding: '20px', borderRadius: '15px', border: '1px solid #18244a', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ color: '#2fe39b', borderBottom: '2px solid #18244a' }}>
          ...
          <tbody>
            {loading ? <tr><td colSpan="5">Loading...</td></tr> : branchesList.map(b => {
              ...
              return (
              <tr key={b.id} style={{ borderBottom: '1px solid #121c38' }}>
                <td style={{ padding: '12px' }}>
                  <b style={{color: '#22d3ee', fontSize: '15px'}}>{b.branch_name}</b><br/>
                  <small style={{color:'#5d7196'}}>Code: {b.branch_code || 'N/A'}</small>
                </td>
                <td style={{ padding: '12px' }}>{b.city}<br/><small style={{color:'#9aadd4'}}>{b.contact_no}</small></td>
                <td style={{ padding: '12px' }}>
                  <span style={{ background: 'rgba(34, 211, 238, 0.1)', color: '#22d3ee', padding: '4px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: 'bold' }}>
                    {activeModsCount} Modules ON
                  </span>
                </td>
                <td style={{ padding: '12px' }}>
                  <button onClick={() => toggleStatus(b)} style={{ background: b.status==='ACTIVE'?'rgba(47, 227, 155,0.1)':'rgba(255, 107, 129,0.1)', color: b.status==='ACTIVE'?'#2fe39b':'#ff6b81', border: `1px solid ${b.status==='ACTIVE'?'#2fe39b':'#ff6b81'}`, padding:'4px 10px', borderRadius:'15px', fontSize:'11px', cursor:'pointer', fontWeight:'bold' }}>{b.status}</button>
                </td>
                <td style={{ padding: '12px' }}>
                  <button className="action-btn" onClick={() => handleEdit(b)}>⚙️ Setup</button>
                  <button className="action-btn" style={{borderColor:'#ff6b81', color:'#ff6b81'}} onClick={() => handleDelete(b.id, b.branch_name)}>🗑️</button>
                </td>
              </tr>
            )})}
          </tbody>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: '#0a1024', width: '100%', maxWidth: '1200px', maxHeight: '95vh', borderRadius: '20px', border: '1px solid #2fe39b', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            
            <div style={{ padding: '15px 25px', display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #18244a' }}>
              <h2 style={{ color: '#2fe39b', margin: 0, fontSize:'20px' }}>⚙️ Setup Branch & Module Limits</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '25px', padding: '25px', overflowY: 'auto' }}>
              ...
              <div style={{ borderRight: '1px solid #18244a', paddingRight: '20px' }}>
                <h4 style={{ color: '#22d3ee', marginTop: 0, borderBottom: '1px solid #18244a', paddingBottom:'5px' }}>📍 BRANCH DETAILS</h4>
                <div style={{marginBottom:'10px'}}><label style={{fontSize:'11px', color:'#9aadd4'}}>BRANCH NAME *</label><input className="input-box" value={formData.branch_name} onChange={e => setFormData({...formData, branch_name: e.target.value})} placeholder="e.g. Siliguri Branch" /></div>
                <div style={{marginBottom:'10px'}}><label style={{fontSize:'11px', color:'#9aadd4'}}>BRANCH CODE</label><input className="input-box" value={formData.branch_code} onChange={e => setFormData({...formData, branch_code: e.target.value})} placeholder="e.g. SLG-01" /></div>
                <div style={{marginBottom:'10px'}}><label style={{fontSize:'11px', color:'#9aadd4'}}>CITY / LOCATION *</label><input className="input-box" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} placeholder="e.g. Siliguri" /></div>
                <div style={{marginBottom:'10px'}}><label style={{fontSize:'11px', color:'#9aadd4'}}>CONTACT NUMBER</label><input className="input-box" value={formData.contact_no} onChange={e => setFormData({...formData, contact_no: e.target.value})} /></div>
                <div style={{marginBottom:'10px'}}><label style={{fontSize:'11px', color:'#9aadd4'}}>FULL ADDRESS</label>
              ...
              <div>
                <h4 style={{ textAlign: 'center', color: '#ffb224', margin: '0 0 15px 0' }}>📊 ALLOWED MODULES FOR THIS BRANCH</h4>
                <p style={{textAlign: 'center', fontSize: '12px', color: '#9aadd4', marginTop: '-10px', marginBottom: '20px'}}>
                ...
                {categories.map(cat => (
                  <div key={cat} style={{ marginBottom: '20px', background: 'rgba(18, 28, 56, 0.4)', padding: '15px', borderRadius: '12px', border: '1px solid #18244a' }}>
                    <div style={{ color: '#2fe39b', fontWeight: 'bold', fontSize: '13px', borderBottom: '1px solid #27395f', paddingBottom: '5px', marginBottom: '10px' }}>{cat}</div>
                      ...
                      {branchModules.filter(m => m.category === cat).map((mod, i) => {
                        ...
                        return (
                          <div key={i} className="module-row">
                            <span style={{ fontWeight: '500', color: mod.isEnabled ? 'white' : '#3d548a' }}>{mod.name}</span>
                            <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', background: mod.isEnabled ? 'rgba(34, 211, 238, 0.1)' : 'transparent', padding: '3px 8px', borderRadius: '5px', border: mod.isEnabled ? '1px solid #22d3ee' : '1px solid #27395f' }}>
                              <input type="checkbox" checked={mod.isEnabled} onChange={e => handleModuleToggle(idx, e.target.checked)} style={{ marginRight: '8px' }} />
                              <span style={{ fontSize: '11px', color: mod.isEnabled ? '#22d3ee' : '#5d7196', fontWeight: 'bold' }}>{mod.isEnabled ? 'ON' : 'OFF'}</span>
                            </label>
                          </div>
                        )
                      })}
            ...
            <div style={{ padding: '15px 30px', textAlign: 'right', background: '#0a1024', borderTop: '1px solid #18244a' }}>
              <button onClick={handleSave} disabled={loading} style={{ background: 'linear-gradient(135deg, #2fe39b, #2fe39b)', color: 'white', padding: '12px 50px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px' }}>
