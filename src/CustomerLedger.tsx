  ...
  return (
    <div className="pt-anim-fade" style={{ padding: 'clamp(12px, 3vw, 30px)', minHeight: '100vh', background: 'radial-gradient(circle at top left, #121c38, #0a1024)', color: 'white', fontFamily: "'Inter', sans-serif", paddingBottom: '60px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px', marginBottom: '18px' }}>
        <div>
          <h1 style={{ fontSize: 'clamp(20px,5vw,30px)', margin: 0, color: '#22d3ee' }}>📖 Customer Khata (Party Ledger)</h1>
          <p style={{ color: '#9aadd4', margin: '4px 0 0', fontSize: '13px' }}>SAB bills ek statement mein — bill register + general ledger. Bill = Cr (lena baki), Receipt = Dr, Balance = Live Outstanding. Har receipt ek RECEIPT voucher hai.</p>
        </div>
      ...
      <div className="pt-card pt-anim-up" style={{ marginBottom: '18px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 180px), 1fr))', gap: '12px' }}>
          <div>
            ...
            <select className="pt-input" style={{ borderColor: '#ffb224' }} value={company} onChange={e => setCompany(e.target.value)}>
              <option value="ALL">— All Companies —</option>
              {companies.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="pt-label" style={{ color: '#22d3ee' }}>👤 Customer *</label>
            <select className="pt-input" style={{ borderColor: '#22d3ee' }} value={cust} onChange={e => setCust(e.target.value)}>
      ...
      {err && (
        <div className="pt-anim-up" style={{ padding: '14px 18px', marginBottom: '14px', borderRadius: '12px', border: '1px solid #ff6b81', background: 'rgba(255, 107, 129,0.08)', color: '#fca5a5', fontSize: '13px' }}>
          ⚠️ {err}
        </div>
      )}

      {!cust ? (
        <div className="pt-anim-up" style={{ textAlign: 'center', padding: '60px 20px', color: '#5d7196', border: '1px dashed #27395f', borderRadius: '16px' }}>
          👆 Customer chunein — uska poora khata (bills, receipts, live outstanding) yahan aa jayega.
        </div>
      ) : loading ? (
        <div className="pt-anim-up" style={{ textAlign: 'center', padding: '60px 20px', color: '#22d3ee', border: '1px dashed #27395f', borderRadius: '16px' }}>
        ...
        <>
          ...
          <div className="pt-stagger" style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', marginBottom: '18px' }}>
            <div className="pt-kpi"><div className="pt-kpi__label" style={{ color: '#22d3ee' }}>Total Billed (Cr)</div><div className="pt-kpi__value" style={{ color: '#22d3ee' }}>₹{inr(ledger.billed)}</div><div className="pt-kpi__sub">{company === 'ALL' ? 'all companies' : company}</div></div>
            <div className="pt-kpi"><div className="pt-kpi__label" style={{ color: '#2fe39b' }}>Received (Dr)</div><div className="pt-kpi__value" style={{ color: '#2fe39b' }}>₹{inr(ledger.received)}</div><div className="pt-kpi__sub">receipts in range</div></div>
            <div className="pt-kpi" style={{ borderColor: ledger.outstanding > 0 ? '#ef444466' : '#10b98166' }}>
              <div className="pt-kpi__label" style={{ color: ledger.outstanding > 0 ? '#ff6b81' : '#2fe39b' }}>💰 Live Outstanding (Lena Baki)</div>
              <div className="pt-kpi__value" style={{ color: ledger.outstanding > 0 ? '#ff6b81' : '#2fe39b' }}>₹{inr(ledger.outstanding)}</div>
          ...
          <div className="pt-card pt-anim-up" style={{ overflowX: 'auto', padding: 'clamp(10px, 2vw, 20px)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', minWidth: '700px' }}>
              <thead><tr style={{ color: '#22d3ee', textAlign: 'left' }}>
                {['Date', 'Particulars (Invoice / Receipt)', 'Debit ₹ (Received)', 'Credit ₹ (Billed)', 'Balance ₹ (Lena Baki)'].map((h, i) => <th key={h} style={{ padding: '10px 8px', borderBottom: '2px solid #27395f', textAlign: i >= 2 ? 'right' : 'left' }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {ledger.opening !== 0 && (
                  <tr style={{ borderBottom: '1px solid #18244a', color: '#9aadd4', fontStyle: 'italic' }}>
                ...
                {ledger.rows.length === 0 ? <tr><td colSpan={5} style={{ textAlign: 'center', padding: '30px', color: '#5d7196' }}>Is range mein koi entry nahi.</td></tr> :
                  ledger.rows.map((r, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #18244a' }}>
                      ...
                      <td style={{ padding: '8px', textAlign: 'right', color: '#2fe39b', fontWeight: r.dr ? 900 : 400 }}>{r.dr ? inr(r.dr) : ''}</td>
                      <td style={{ padding: '8px', textAlign: 'right', color: '#22d3ee', fontWeight: r.cr ? 900 : 400 }}>{r.cr ? inr(r.cr) : ''}</td>
                      <td style={{ padding: '8px', textAlign: 'right', fontWeight: 900, color: r.balance > 0 ? '#ff6b81' : '#2fe39b' }}>{inr(r.balance)}</td>
                    </tr>
                  ))}
                <tr style={{ background: 'rgba(34, 211, 238,0.06)', fontWeight: 900 }}>
                  <td style={{ padding: '10px 8px' }} colSpan={2}>TOTALS</td>
                  <td style={{ padding: '10px 8px', textAlign: 'right', color: '#2fe39b' }}>₹{inr(ledger.received)}</td>
                  <td style={{ padding: '10px 8px', textAlign: 'right', color: '#22d3ee' }}>₹{inr(ledger.billed)}</td>
                  <td style={{ padding: '10px 8px', textAlign: 'right', color: ledger.outstanding > 0 ? '#ff6b81' : '#2fe39b', fontSize: '15px' }}>₹{inr(ledger.outstanding)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <BottomSheet open={showPay} onClose={() => setShowPay(false)} title={`💰 Receipt Entry — ${custName || 'select customer'}`} accent="#2fe39b" maxWidth={640}>
        <div className="pt-anim-fade">
          <div style={{ marginBottom: '14px', fontSize: '13px', color: '#9aadd4' }}>
            Khata: <b style={{ color: '#22d3ee' }}>{custName || '—'}</b> · Company: <b style={{ color: '#ffb224' }}>{company === 'ALL' ? '⚠ chunein (required)' : company}</b>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' }}>
            <div><label className="pt-label" style={{ color: '#2fe39b' }}>Amount (₹) *</label><input type="number" inputMode="decimal" className="pt-input" style={{ borderColor: '#2fe39b', fontSize: '18px', fontWeight: 'bold' }} value={pay.amount} onChange={e => setPay({ ...pay, amount: e.target.value })} placeholder="0.00" /></div>
          ...
          <div style={{ marginTop: '14px' }}>
            <label className="pt-label" style={{ color: '#2fe39b' }}>Paisa kis account me aaya? *</label>
            <select className="pt-input" style={{ borderColor: '#2fe39b' }} value={pay.account} onChange={e => setPay({ ...pay, account: e.target.value })}>
            ...
            <div style={{ marginTop: '8px', fontSize: '11px', color: '#9aadd4' }}>
              Posts <b style={{ color: '#2fe39b' }}>Dr {pay.account || 'the account you select'}</b>
              {' / '}<b style={{ color: '#ff8b9c' }}>Cr Debtors: {custName || 'customer'}</b>
            </div>
          </div>
          <div style={{ marginTop: '14px' }}>
            ...
            <div style={{ marginTop: '6px', fontSize: '11px', color: '#5d7196' }}>
