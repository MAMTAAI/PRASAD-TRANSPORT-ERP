...
export default function WebSettings() {
  ...
  // Custom Input Style
  const inputStyle = { width: '100%', padding: '12px', background: 'rgba(18, 28, 56, 0.6)', border: '1px solid #27395f', color: '#fff', borderRadius: '8px', marginBottom: '15px', marginTop: '5px', boxSizing: 'border-box' as const, outline: 'none', fontSize: '13px' };
  const labelStyle = { color: '#9aadd4', fontSize: '11px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '1px' as const };

  return (
    <div className="cms-container" style={{ padding: '30px', minHeight: '100vh', background: 'radial-gradient(circle at top right, #121c38, #0a1024)', fontFamily: "'Inter', sans-serif" }}>
      
      {/* 📱 RESPONSIVE CSS STYLES */}
      <style>{`
        .glass-card { background: rgba(24, 36, 74, 0.4); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 16px; backdrop-filter: blur(10px); box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
        ...
        .gradient-text { background: linear-gradient(135deg, #22d3ee, #818cf8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
      ...
      <div className="header-flex" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', paddingBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div>
          ...
          <p style={{ color: '#9aadd4', margin: '5px 0 0 0', fontSize: '14px', fontWeight: '500' }}>Manage all text, images, and dynamic content of your Public Website directly from here.</p>
        </div>
        <button onClick={restoreDefaults} style={{ background: 'rgba(255, 107, 129, 0.1)', color: '#ff6b81', border: '1px solid rgba(255, 107, 129,0.5)', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '12px', textTransform: 'uppercase' }}>🔄 Restore Defaults</button>
      </div>
      ...
      <div className="main-layout">
        ...
        <div style={{ position: 'sticky', top: '30px', alignSelf: 'start', order: window.innerWidth < 1000 ? 1 : 2 }}>
          ...
          <div className="live-preview-box" style={{ width: '100%', height: '80vh', borderRadius: '24px', overflow: 'hidden', position: 'relative', border: '4px solid #18244a', boxShadow: '0 25px 50px rgba(0,0,0,0.8)' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(10, 16, 36,0.98) 0%, rgba(10, 16, 36,0.7) 40%, rgba(10, 16, 36,0.2) 100%)', zIndex: 2 }}></div>
            ...
            <div style={{ position: 'relative', zIndex: 3, padding: '30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start' }}>
                ...
                <p style={{ color: '#dde5f4', fontSize: '13px', lineHeight: '1.6', maxWidth: '90%' }}>{data.desc}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '25px', order: window.innerWidth < 1000 ? 2 : 1 }}>
          ...
          <div className="glass-card" style={{ padding: '25px', borderTop: '4px solid #22d3ee' }}>
            <h3 style={{ margin: '0 0 20px 0', color: '#22d3ee', fontSize: '18px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>🖼️ 1. Hero Section (Home Page)</h3>
            ...
            <div style={{ marginTop: '15px', padding:'20px', background:'rgba(0,0,0,0.3)', borderRadius:'12px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <input type="file" multiple accept="image/*" onChange={handleImageUpload} style={{ width: '100%', padding: '12px', color: '#fff', borderRadius: '8px', marginBottom: '15px', border: '1px dashed #3d548a', marginTop: '10px', background: 'rgba(18, 28, 56,0.5)' }} disabled={uploadingImg} />
              {uploadingImg && <p style={{color: '#22d3ee', fontSize: '12px', marginTop: 0, fontWeight: 'bold'}}>⏳ Uploading to secure server... please wait.</p>}

              <div style={{display:'flex', gap:'10px', overflowX:'auto', paddingBottom: '5px'}}>
                  {data.bgImages.map((img, i) => (
                      <div key={i} style={{position:'relative', minWidth:'100px', height:'70px', borderRadius:'8px', overflow:'hidden', border: currentSlide === i ? '2px solid #22d3ee' : '2px solid transparent', boxShadow: '0 4px 10px rgba(0,0,0,0.5)'}}>
                          <img src={img} style={{width:'100%', height:'100%', objectFit:'cover'}} alt="slider" />
                          <button onClick={()=>removeImage(i)} style={{position:'absolute', top:4, right:4, background:'rgba(255, 107, 129,0.9)', border:'none', color:'white', borderRadius:'50%', width:'22px', height:'22px', cursor:'pointer', fontSize:'10px', fontWeight:'bold'}}>✕</button>
                      </div>
                  ))}
              </div>
            </div>
          ...
          {/* 2. ABOUT US */}
          <div className="glass-card" style={{ padding: '25px', borderTop: '4px solid #ffb224' }}>
            <h3 style={{ margin: '0 0 20px 0', color: '#ffb224', fontSize: '18px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>🏢 2. About Us Section</h3>
          ...
          <div className="glass-card" style={{ padding: '25px', borderTop: '4px solid #8b5cf6' }}>
            ...
            <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {data.fleetCards.map((card, index) => (
                <div key={card.id} style={{ background: 'rgba(0,0,0,0.3)', padding: '15px', borderRadius: '12px', border: '1px solid #27395f', position: 'relative' }}>
                  <button onClick={() => removeFleetCard(card.id)} style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(255, 107, 129, 0.2)', color: '#ff6b81', border: '1px solid #ff6b81', borderRadius: '4px', cursor: 'pointer', fontSize: '10px', padding: '4px 8px', fontWeight: 'bold' }}>DELETE</button>
          ...
          {/* 4. CONTACT & SUPPORT */}
          <div className="glass-card" style={{ padding: '25px', borderTop: '4px solid #2fe39b' }}>
            <h3 style={{ margin: '0 0 20px 0', color: '#2fe39b', fontSize: '18px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>📞 4. Contact & Footer Settings</h3>
